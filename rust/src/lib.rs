...
use silentpayments::{
    bitcoin_hashes::hex::DisplayHex,
    receiving::{Label, Receiver},
    secp256k1::{self, PublicKey, SecretKey, XOnlyPublicKey},
    utils::receiving::calculate_ecdh_shared_secret,
    Network,
};
use std::{collections::HashMap, ffi::CString, os::raw::c_char, slice};
...
pub extern "C" fn api_scan_outputs(data: *const ParamData) -> *mut i8 {
    ...
    let mut sp_receiver =
        Receiver::new(0, b_scan.public_key(&secp), B_spend, change_label, network).unwrap();
